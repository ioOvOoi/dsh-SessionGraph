...
/* T5: 交互原子(切枝确认按钮组) + 失败提示 */
.sg-act{cursor:pointer;transition:opacity .15s ease;}
.sg-act:hover{opacity:.85;}
.sg-act-disabled{pointer-events:none;opacity:.35;}
.sg-act-msg{font-family:ui-monospace,Consolas,monospace;font-size:9px;letter-spacing:.03em;pointer-events:none;}
`)
    ...
    const GraphView = ({ sessionId, base, cursor, onCollapse }) => {
        ...
        if (n.category === 'turn') {
          ...
          children.push(React.createElement('g', {
            ...
            onClick: (e) => {
              ...
              if (jumpMsgTimerRef.current) { jumpMsgTimerRef.current(); jumpMsgTimerRef.current = null }
              interactRef.current = Date.now()
              // 交互原子锚定节点位置右下方(节点坐标 + 半径偏移),不跟随鼠标、不覆盖节点
              setPendingJump({ id: n.id, anchorSeq: n.seq, x: p.x, y: p.y, r: r })
              setSelectedId(n.id)
              setPulseKey((k) => k + 1)
              jumpNode(n, items)
            },
      ...
      // T5: 交互原子组渲染 — 复用工具簇扇形算法(fanOut 同款几何)
      //   交互原子 = 临时工具小原子:挂节点左侧 x = 节点x - (FAN_X0 + k*FAN_DX),纵向扇形 fanY
      //   (与 pwsh/cordis_define 等工具原子完全一致的相对位置与距离)
      if (pendingJump) {
        ...
        const a0 = fanPos(0) // ⇄ 确认原子(第 0 个工具位)
        const a1 = fanPos(1) // ✕ 取消原子(第 1 个工具位)
        children.push(React.createElement('g', { key: 'act-grp' },
          // 「⇄ 切到这里继续」确认原子
          React.createElement('circle', {
            className: 'sg-act' + (disabled ? ' sg-act-disabled' : ''),
            cx: a0.x, cy: a0.y, r: 11,
            fill: '#f5edda', stroke: '#b2954a', strokeWidth: 1.5,
            onClick: confirmJump,
            title: disabled ? '当前回合进行中' : '切到这里继续',
          }),
          React.createElement('text', {
            ...
            fill: disabled ? '#b0a89a' : '#b2954a',
          }, '⇄'),
          // 「✕ 取消」原子 — 第 1 个工具位
          React.createElement('circle', {
            className: 'sg-act',
            cx: a1.x, cy: a1.y, r: 9,
            fill: '#e8e4de', stroke: '#b0a89a', strokeWidth: 1,
            onClick: cancelJump,
            title: '取消',
          }),
          React.createElement('text', {
      ...
      // T5: jump 失败提示文案 — 交互原子组下方短暂显示
      if (jumpMsg && pendingJump) {
        children.push(React.createElement('text', {
          key: 'act-msg',
          className: 'sg-act-msg',
          x: pendingJump.x - (LAYOUT.FAN_X0 + LAYOUT.FAN_DX),
          y: pendingJump.y + LAYOUT.FAN_DY + 18,
          textAnchor: 'start',
          fill: '#a0522d',
        }, jumpMsg))
      }
