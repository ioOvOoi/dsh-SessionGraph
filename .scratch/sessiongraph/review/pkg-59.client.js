...
`)
    ...
    // 扁平 RPC 节点 → 内部 meta 结构(投影节点本身带 meta,直接透传)
    const normalizeNode = (n) => ({
      ...
      text: n.text || '',
      meta: {
        turn: n.turn == null ? null : n.turn,
        step: n.step == null ? null : n.step,
        name: n.name || null,
        result: !!n.result,
        error: !!n.error,
        think: !!n.think,
        plugin: n.plugin || null,
      },
    })
    ...
    const buildLayout = (items, roundOf, turnRound, roundSnippet, effFoldKey) => {
      ...
      // 引力漂移:每节点按 id 散列向两侧漂移,相邻漂移量受限 → 蜿蜒而非直线
      const lateral = (n, r) => {
        const unit = (hashOf(n.id) % 1000) / 1000 - 0.5
        return unit * (r + 8) * 3
      }
      const spineIdx = {}
      const restPos = {}
      ...
      const nodeH = (n) => 2 * rOfChain(n) + 26
      let prevX = 0
      chain.forEach((n, i) => {
        const r = rOfChain(n)
        let x = i === 0 ? lateral(n, r) : Math.max(prevX - 34, Math.min(prevX + 34, lateral(n, r)))
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        restPos[it.id] = { x: -30, y }
      }

      // 工具簇:围绕所属助手向左侧扇形散开(纵向拉开展)
      const childMap = {}
      const byId = {}
      let lastAsst = null
      ...
      for (const aid of Object.keys(childMap)) {
        ...
        arr.forEach((tid, k) => {
          stackIdx[tid] = k
          const t = byId[tid]
          const fanY = n === 1 ? 0 : (k - (n - 1) / 2) * 18
          restPos[tid] = {
            x: bp.x - (26 + k * 22 + (t && t.category === 'switch' ? 6 : 0)),
            y: bp.y + fanY + (t && t.category === 'switch' ? 8 : 0),
          }
        })
      }
      ...
      for (let i = 1; i < chain.length; i++) {
        edges.push({ a: chain[i - 1].id, b: chain[i].id, curve: (i % 2 === 0 ? 1 : -1) * 14 })
      }
    ...
    const GraphView = ({ sessionId, base, cursor, onCollapse }) => {
      ...
      const runs = (switches || []).slice().sort((a, b) => a.startedAt - b.startedAt)
      ...
      const defaultFoldForKey = (k) => {
        ...
        return !isNaN(idx) && idx >= 0 ? idx < roundsInOrder.length - 2 : false
      }
      ...
      React.useEffect(() => {
        ...
        if (typeof ResizeObserver !== 'undefined') {
          ro = new ResizeObserver(() => {
            ...
            if (timer) timer.timeout(fire, 150)
            else fire()
          })
          ro.observe(el)
        }
        ...
        if (n.category === 'turn') {
          ...
          if (f) {
            children.push(React.createElement('g', {
              ...
              onClick: () => { toggleFold(rk); jumpNode(n, items) },
          ...
          children.push(React.createElement('g', {
            ...
            React.createElement('circle', { cx: p.x, cy: p.y, r: r + 10, fill: 'transparent' }),
            React.createElement('circle', { cx: p.x, cy: p.y, r: r, fill: cat.fill, stroke: cat.stroke, strokeWidth: 1.5, strokeDasharray: '3 2' }),
            showLabels ? React.createElement('text', { x: p.x, y: p.y - r - 5, className: 'sg-label', textAnchor: 'middle' },
              '委派→' + String(n.meta.childId).slice(0, 6)) : null,
          ))
        } else if (n.category === 'tool') {
          children.push(React.createElement('g', {
            key: n.id, className: cls + (showBob ? ' sg-bob' : ''), style: { animationDelay: ((L.stackIdx[n.id] || 0) % 5) * 0.25 + 's' },
            ...
            React.createElement('circle', {
              cx: p.x, cy: p.y, r: r,
              fill: neighborSet.has(n.id) || sel ? '#a9a49c' : cat.fill,
              stroke: sel ? '#4a4a4a' : 'none', strokeWidth: 1.5,
            }),
            showLabels ? React.createElement('text', { x: p.x, y: p.y + r + 10, className: 'sg-label-text', textAnchor: 'middle' },
          ...
          children.push(React.createElement('g', {
            ...
            onClick: () => { setSelectedId(n.id); setPulseKey((k) => k + 1); jumpNode(n, items) },
            onMouseEnter: () => setHoverId(n.id),
            onMouseLeave: () => setHoverId(null),
            title: cat.label + ' #' + n.seq + ' ' + (n.text || '').slice(0, 80),
          },
            React.createElement('circle', { cx: p.x, cy: p.y, r: r + 10, fill: 'transparent' }),
            ...
            React.createElement('circle', {
              cx: p.x, cy: p.y, r: r,
              fill: neighborSet.has(n.id) && !sel ? '#6f6a62' : cat.fill,
    ...
    const SlimOverlay = ({ useSessions }) => {
      ...
      React.useEffect(() => {
        ...
        check()
        const timer = ctx.get('timer')
        const dispose = timer ? timer.interval(check, 400) : null
      ...
      React.useEffect(() => {
        ...
        fetchData()
        const timer = ctx.get('timer')
        const dispose = timer ? timer.interval(fetchData, 1000) : null
        return () => { alive = false; if (dispose) dispose() }
      }, [collapsed, current])

      if (!collapsed) return null

      const runs = ((data && data.switches) || []).slice().sort((a, b) => a.startedAt - b.startedAt)
          ...
          React.createElement('div', { className: 'sg-slim-track' },
            items.map((n) => {
              ...
              return React.createElement('div', {
                ...
                title: (n.category === 'tool' ? (n.meta.name || '工具') : CAT[n.category] ? CAT[n.category].label : n.category) + ' ' + snippet(n.text, 20),
              })
            }),
          ),
        ),
