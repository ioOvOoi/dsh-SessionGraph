...
`)
    ...
    const GraphView = ({ sessionId, base, cursor, onCollapse }) => {
      ...
      // T5: 交互原子组渲染 — pendingJump 非空时,锚定节点右下方(半径 + 12 偏移,不覆盖节点)
      if (pendingJump) {
        const ax = pendingJump.x + pendingJump.r + 12
        const ay = pendingJump.y + pendingJump.r + 12
      ...
      if (jumpMsg && pendingJump) {
        children.push(React.createElement('text', {
          ...
          x: pendingJump.x + pendingJump.r + 12,
          y: pendingJump.y + pendingJump.r + 34,
          textAnchor: 'start',
          fill: '#a0522d',
        }, jumpMsg))
      }
