return {
  apply(ctx) {
    ...
    // 侧边栏数据 RPC:返回全部节点(文本截断 60)+ callId,供收起窄条浮层轮询
    harness.handle('sessiongraph.get', async (args) => {
      ...
      const nodes = graph.nodes.map((n) => ({
        ...
        callId: n.meta.callId == null ? null : String(n.meta.callId),
        result: n.meta.result || false,
        error: n.meta.error || false,
    ...
    console.log('sessiongraph: 投影 v5 + RPC(switches/toolinfo/get-全量+callId)已注册')
    ...
    console.log('sessiongraph: 调试工具 sessiongraph_debug 已注册')
  },
}
