...
`;
    ...
    // 金环移除定时器:模块级持有,下次跳转前清理,防堆积(红队 P2)
    let flashTimer = null
    const scrollToKey = (key) => {
      ...
      flashTimer = setTimeout(() => el.classList.remove('sg-jump-flash'), 1700)
    }
    ...
    //   选项对象化(替代 12 个位置参数,防对位错误)
    const renderToolAtom = (opts) => {
      const { x, y, r, fill, stroke, dash, iconText, title, onClick, disabled, key, showLabel, hoverCls } = opts
      const elems = []
    ...
    //   - timer → 原生 setTimeout
    //   - 操作原子组仅保留 ± 折叠/展开 + ✕ 关闭
    const GraphView = ({ sessionId, base, cursor, onCollapse, switches, toolInfo }) => {
      ...
      // 从 props 获取 switches/toolInfo (graph 投影 state, 引用稳定)
        ...
        if (n.category === 'turn') {
          ...
          children.push(React.createElement('g', {
            ...
            ...renderToolAtom({
              x: p.x, y: p.y, r: r,
              fill: cat.fill,
              stroke: cat.stroke, dash: '3 2',
              iconText: '委派→' + String(n.meta.childId).slice(0, 6),
              title: '委派 → ' + n.meta.childId + ' · ' + n.meta.provider + (n.meta.stopReason ? ' · ' + reasonText(n.meta.stopReason) : ' · 运行中'),
              onClick: () => {
                setSelectedId(n.id)
                jumpNode(n, items)
          ...
          children.push(React.createElement('g', {
            ...
            ...renderToolAtom({
              ...
              stroke: sel ? '#4a4a4a' : undefined,
              dash: undefined,
              iconText: labelOf(n, toolInfo),
              title: cat.label + ' ' + (n.meta.name || '') + ' #' + n.seq + (n.meta.result ? (n.meta.error ? ' ✗失败' : ' ✓完成') : ' …运行中'),
      ...
      if (opTarget) {
        ...
        if (p) {
          ...
          children.push(React.createElement('g', { key: 'act-fold' },
            ...renderToolAtom({
              x: actPos(baseK).x, y: actPos(baseK).y, r: 5,
              fill: foldDisabled ? '#d9d9d9' : '#c9c9c9',
              stroke: undefined, dash: undefined,
              iconText: foldIcon,
              title: foldTitle,
              onClick: (e) => {
                e.stopPropagation()
          ...
          children.push(React.createElement('g', { key: 'act-close' },
            ...renderToolAtom({
              x: actPos(baseK + 1).x, y: actPos(baseK + 1).y, r: 5,
              fill: '#c9c9c9',
              stroke: undefined, dash: undefined,
              iconText: '✕',
              title: '取消',
              onClick: closeOp, disabled: false,
              key: 'act-c',
              showLabel: true, hoverCls: 'sg-op',
            }),
          ))
    ...
    // 拖拽/父级重渲染期间跳过重渲染 — switches/toolInfo 引用由投影 state 稳定化(内容未变引用不变)
    const GraphViewMemo = React.memo(GraphView, (a, b) =>
      a.sessionId === b.sessionId &&
      a.base === b.base &&
      a.cursor === b.cursor &&
      a.switches === b.switches &&
      a.toolInfo === b.toolInfo
    )
    ...
    // ===== details 列(真实布局列) =====
    //   switches/toolInfo 由 graph 投影 state 提供(引用稳定),
    //   作为 props 传给 GraphView;同时把 nodes/cursor/switches 写入
    //   _sharedGraphData 供 SlimOverlay 订阅。
    const GraphPanel = ({ sessionId, useProjection }) => {
      ...
      const switches = graph && Array.isArray(graph.switches) ? graph.switches : []
      const toolInfo = graph && typeof graph.toolInfo === 'object' ? graph.toolInfo : null

      // 将图数据写入模块级共享状态(SlimOverlay 订阅)
      React.useEffect(() => {
        _sharedGraphData.update(base, cursor, switches)
      }, [base, cursor, switches])
